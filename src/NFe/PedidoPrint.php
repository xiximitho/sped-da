...
class PedidoPrint extends Common
{
    protected $papel;
    ...
    public function montaDANFE(
        ...
        if(getenv("QRCODE_MAPS") == 1 && $this->venda->endereco_id != NULL){
            $qrCodeExiste = 50;
        }

        $tamPapelVert = (100 + $qrCodeExiste) + 16 + (($qtdItens - 1) * $hMaxLinha) + ($qtdPgto * $hLinha);
        ...
        $y = $this->pTotalDANFE($x, $y + ($this->totalItens), $hTotal);
        // //COLOCA PAGAMENTOS
...
{
    $config = ConfigNota::where('empresa_id', $this->venda->empresa_id)->first();
...
protected function pCabecalhoSecundarioDANFE($x = 0, $y = 0, $h = 0)
{
    ...
    $w = ($maxW*1);
    $hBox1 = 7;
    $texto = "PEDIDO ID: " . $this->venda->id;
    $aFont = array('font'=>$this->fontePadrao, 'size'=>10, 'style'=>'B');
    $this->pTextBox($x, $y, $w, $hBox1, $texto, $aFont, 'C', 'C', 0, '', false);
    $hBox2 = 4;
    $yBox2 = $y + $hBox1;
    $texto = $this->venda->cliente->nome . ' ' .$this->venda->cliente->sobre_nome;
    $aFont = array('font'=>$this->fontePadrao, 'size'=>10, 'style'=>'B');
...
protected function pTotalDANFE($x = 0, $y = 0, $h = 0)
{
    ...
    $texto = "R$ " . number_format($this->venda->valor_total - $this->venda->valor_entrega, 2, ',', '.');
    $aFont = array('font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B');
    $this->pTextBox($xValor, $yTotal, $wColDir, $hLinha, $texto, $aFont, 'T', 'R', 0, '', false);
    
    $yDesconto = $y + ($hLinha*7);
    ...
    $texto = "R$ " . number_format($this->venda->valor_total, 2, ',', '.');
    $aFont = array('font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B');
...
/*     $texto = "Troco Para";
    ...
    $texto = "R$ " . number_format($this->venda->troco_para, 2, ',', '.');
    $aFont = array('font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B');
...
/*     $yTotalFinal = $y + ($hLinha*9);
    ...
    $aFont = array('font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B');
    $this->pTextBox($x, $yTotalFinal, $wColEsq, $hLinha, $texto, $aFont, 'T', 'L', 0, '', false);
    $texto = Carbon::parse($this->venda->data_registroo)->format('d/m/Y');
    $aFont = array('font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B');
    $this->pTextBox($xValor, $yTotalFinal, $wColDir, $hLinha, $texto, $aFont, 'T', 'R', 0, '', false); */
    ...
    $aFont = array('font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B');
    $this->pTextBox($x, $yTotalFinal, $wColEsq, $hLinha, $texto, $aFont, 'T', 'L', 0, '', false);
    $texto = Carbon::parse($this->venda->data_registro)->format('H:i');
    $aFont = array('font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B');
    $this->pTextBox($xValor, $yTotalFinal, $wColDir, $hLinha, $texto, $aFont, 'T', 'R', 0, '', false);


    $config = DeliveryConfig::first();
    $dataRegistro = $this->venda->data_registro;
    // $entrega = strtotime("+" . $config->tempo_medio_entrega . " minutes", strtotime($dataRegistro));
    $entrega = strtotime("+" . $this->venda->tempo_entrega . " minutes", strtotime($dataRegistro));
    ...
    $texto = date('H:i', $entrega);
    $aFont = array('font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B');
    $this->pTextBox($xValor, $yTotalFinal, $wColDir, $hLinha, $texto, $aFont, 'T', 'R', 0, '', false);


    $taxaEntraga = $this->venda->calculaFrete();
    ...
    $yTotalFinal = $y + ($hLinha*12);
    $texto = "Observação";
    $aFont = array('font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B');
    $this->pTextBox($x, $yTotalFinal, $wColEsq, $hLinha, $texto, $aFont, 'T', 'L', 0, '', false);
    $texto = $this->venda->observacao == '' ? '--' : $this->venda->observacao;
    $aFont = array('font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B');
    $this->pTextBox(28, $yTotalFinal, 50, $hLinha, $texto, $aFont, 'T', 'R', 0, '', false);


    if($this->venda->endereco != null){
        $yTotalFinal = $y + ($hLinha*14);
        $texto = "Endereço";
        $aFont = array('font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B');
        $this->pTextBox($x, $yTotalFinal, $wColEsq, $hLinha, $texto, $aFont, 'T', 'L', 0, '', false);
        $texto = $this->venda->endereco->rua .", ".$this->venda->endereco->numero . " - ". 
        $this->venda->endereco->_bairro->nome . " | " . $this->venda->endereco->referencia . " (" . $this->venda->telefone . ")";
        $aFont = array('font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B');
        $this->pTextBox(20, $yTotalFinal, 58, $hLinha, $texto, $aFont, 'T', 'R', 0, '', false);
    }else{
        $yTotalFinal = $y + ($hLinha*14);
        $texto = "Endereço";
        $aFont = array('font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B');
        $this->pTextBox($x, $yTotalFinal, $wColEsq, $hLinha, $texto, $aFont, 'T', 'L', 0, '', false);
        $texto = "Cliente vai retirar no local" ."\n (" . $this->venda->telefone . ")";
        $aFont = array('font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B');
        $this->pTextBox(35, $yTotalFinal, 43, $hLinha, $texto, $aFont, 'T', 'R', 0, '', false);
    }
...
protected function pConsumidorDANFE($x = 0, $y = 0, $h = 0)
{
    ...
    protected function pQRDANFE($x = 0, $y = 0, $h = 0)
    {
        ...
        if(file_exists($public.'rotas/'.$this->venda->id.'.png') && getenv("QRCODE_MAPS") == 1){
