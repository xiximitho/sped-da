...
class CupomPedidoIfood extends Common
{
	protected $papel;
    ...
    protected function cabecalhoSecundarioDANFE($x = 0, $y = 0, $h = 0)
    {
    	...
    	$w = ($maxW*1);
    	$hBox1 = 7;
    	$texto = "PEDIDO IFOOD #" . $this->pedido->id;
    	...
    	if($this->pedido->nome_cliente != ''){
    		$hBox1 = 20;
    		$texto = "Cliete: ". $this->pedido->nome_cliente;
    	...
    	if($this->pedido->nome_cliente != ''){
    		$hBox1 = 25;
    		$texto = "Telefone: ". $this->pedido->telefone_cliente;
    		$aFont = array('font'=>$this->fontePadrao, 'size'=>8, 'style'=>'B');
    		$this->pdf->textBox($x, $y, $w, $hBox1, $texto, $aFont, 'C', 'C', 0, '', false);
    		$hBox2 = 5;
    	}

    	if($this->pedido->pedido_id != ''){
    		$hBox1 = 30;
    		$texto = "ID: ". $this->pedido->pedido_id;
    		$aFont = array('font'=>$this->fontePadrao, 'size'=>8, 'style'=>'B');
    		$this->pdf->textBox($x, $y, $w, $hBox1, $texto, $aFont, 'C', 'C', 0, '', false);
    		$hBox2 = 5;
    	}
    ...
    protected function produtosDANFE($x = 0, $y = 0, $h = 0)
    {
    	...
    	if ($qtdItens > 0) {
    		foreach ($this->pedido->itens as $p) {
    			...
    			$nomeP = '';
    			$nomeP = $p->nome_produto;
    			if(sizeof($p->adicionais) > 0){
    				$nomeP .= ", adicionais: ";
    				foreach($p->adicionais as $a){
    					$nomeP .= "$a->quantidade x $a->nome - R$ " .moeda($a->valor_unitario). " | ";
    				}

    				$nomeP = substr($nomeP, 0, strlen($nomeP)-2);
    			}
    			$xProd      = $nomeP;
    			...
    			$qCom       = $p->quantidade;
    			$uCom       = 'UN';
    			$vUnCom     = number_format($p->valor_unitario, 2, ",", ".");

    			$valorProduto = $p->total;
    ...
    protected function totalDANFE($x = 0, $y = 0, $h = 0)
    {
    	...
    	foreach($this->pedido->payments as $pay){
    		$texto = \App\Models\PagamentoPedidoIfood::getFormPay($pay->forma_pagamento);
    		$aFont = ['font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B'];
    		$this->pdf->textBox($x, $yTotalFinal, $wColEsq, $hLinha, $texto, $aFont, 'T', 'L', 0, '', false);
    		$totalComFrete = $this->pedido->valor_total;
    		$texto = "R$ " . number_format($pay->valor, 2, ',', '.');
    		$aFont = ['font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B'];
    		$this->pdf->textBox($xValor, $yTotalFinal, $wColDir, $hLinha, $texto, $aFont, 'T', 'R', 0, '', false);
    		$contL++;
    		$yTotalFinal = $y + ($hLinha*$contL);
    	...
    	$texto = $this->pedido->endereco . " - " . $this->pedido->bairro . " | " . $this->pedido->cep;
    	$aFont = ['font'=>$this->fontePadrao, 'size'=>9, 'style'=>'B'];
